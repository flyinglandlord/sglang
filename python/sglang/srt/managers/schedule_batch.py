    ...
    def prepare_for_resume_decode(self):
        ...
        for i in range(bs):
            assert len(self.tree_cache.entries[self.reqs[i].rid].value) == self.seq_lens[i], \
                f"something wrong with the tree cache: token slots number not equal {reqs[i].rid} " \
                f"{self.tree_cache.entries[self.reqs[i].rid].value.shape} and {self.seq_lens[i]}"
